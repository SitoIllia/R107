...
if date1 is not None and date2 is not None:
    if date1 > date2:
        ...
        print(f"\nLe fichier le plus récent est {fichier2}, modifié le {datetime.fromtimestamp(date2)}")
    else:
        print(f"\nLes deux fichiers ont été modifiés à la même date : {datetime.fromtimestamp(date1)}")
